...
import InputComponent from "../../component/InputComponet/InputComponent"
...
const handleChangeAddress = () =>{
  ...
  const priceMemo = useMemo(() => {
    const result =
      ...
      order.orderItemsSlected.reduce((total, cur) => {
  ...
  const diliveryPriceMemo = useMemo(() => {
    if(priceMemo >= 20000 && priceMemo < 500000){
      return 10000;
    }else if(priceMemo >= 500000 || order && order.orderItemsSlected && order.orderItemsSlected.length
      === 0) {
      return 0;
    } else {
      return 20000;
  ...
  const handleAddCard = () => {
    if (!order || !order.orderItemsSlected || !order.orderItemsSlected.length) {
                        ...
                        <WrapperCountOrder>
                          <button
                            ...
                            onClick={() =>
                              handleChangeCount(
                                "decrease",
                                order && order.product, order && order.amount===1
                              )
                            }
                          >
                            <MinusOutlined
                          ...
                          <WrapperInputNumber
                            ...
                            size="small" min={1} max={order && order.countInstock}
                          />
                          <button
                            style={{
                              border: "none",
                            ...
                            onClick={() =>
                              handleChangeCount(
                                "increase",
                                order && order.product
                                ,order && order.amount === order.countInstock,
